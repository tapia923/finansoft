...
namespace FE.pdf
{
    public class NotaCreditoPdf : DocumentoPdf
    {
        public CreditNoteType CreditNote { get; private set; }

        public override string Tipo { get; } = TIPO_NOTA_CREDITO;

        public NotaCreditoPdf(CreditNoteType creditNote)
        {
        ...
        public override byte[] Generar()
        {
            ...
            string nombreEmisor = CreditNote.AccountingSupplierParty.Party.PartyName[0].Name.Value;
            ...
            var nitEmisor = CreditNote.AccountingSupplierParty.Party.PartyTaxScheme[0].CompanyID.Value;
            var dvEmisor = CreditNote.AccountingSupplierParty.Party.PartyTaxScheme[0].CompanyID.schemeID;
            ...
            var direccionEmisor = CreditNote.AccountingSupplierParty.Party.PhysicalLocation.Address.Country.Name.Value + ", " +
                CreditNote.AccountingSupplierParty.Party.PhysicalLocation.Address.CountrySubentity?.Value + ", " +
                CreditNote.AccountingSupplierParty.Party.PhysicalLocation.Address.CityName?.Value + " \r\n" +
                CreditNote.AccountingSupplierParty.Party.PhysicalLocation.Address.AddressLine[0]?.Line.Value;
            ...
            var correoEmisor = CreditNote.AccountingSupplierParty.Party.Contact?.ElectronicMail?.Value;
            ...
            var telefonoEmisor = CreditNote.AccountingSupplierParty.Party.Contact?.Telephone?.Value;
            ...
            var vNumeroNota = CreditNote.ID.Value;
            ...
            if (string.IsNullOrEmpty(TextoQR)) TextoQR = CreditNote.UUID.Value;
            ...
            var nombreAdquiriente = CreditNote.AccountingCustomerParty[0].Party.PartyName[0].Name.Value;
            var identificacionAdquiriente = CreditNote.AccountingCustomerParty[0].Party.PartyTaxScheme[0].CompanyID.Value;
            var direccionAdquiriente = CreditNote.AccountingCustomerParty[0].Party.PhysicalLocation.Address.Country.Name.Value + ", " +
                CreditNote.AccountingCustomerParty[0].Party.PhysicalLocation.Address.CountrySubentity?.Value + ", " +
                CreditNote.AccountingCustomerParty[0].Party.PhysicalLocation.Address.CityName?.Value + ", " +
                CreditNote.AccountingCustomerParty[0].Party.PhysicalLocation.Address.AddressLine[0]?.Line.Value;
            var correoAdquiriente = CreditNote.AccountingCustomerParty[0].Party.Contact?.ElectronicMail?.Value;
            var telefonoAdquiriente = CreditNote.AccountingCustomerParty[0].Party.Contact?.Telephone?.Value;
            ...
            var fechaEmision = CreditNote.IssueDate.Value;
            var horaEmision = CreditNote.IssueTime.Value.Split(new char[] { '+', '-' })[0];
            ...
            var facturaReferencia = CreditNote.BillingReference[0]?.InvoiceDocumentReference?.ID.Value;
            ...
            var cude = CreditNote.UUID.Value;
            var iCude = "CUDE\r\n" + cude;
            ...
            // agregar líneas
            var totalLineas = CreditNote.CreditNoteLine.Count();
            ...
            var descuentosDetalle = CreditNote.CreditNoteLine.Sum(l => l.AllowanceCharge?.Sum(ac => !ac.ChargeIndicator.Value ? ac.Amount.Value : 0M));
            var subtotalPU = CreditNote.LegalMonetaryTotal.LineExtensionAmount.Value + (descuentosDetalle.HasValue ? descuentosDetalle.Value : 0M);
            ...
            var recargosDetalle = CreditNote.CreditNoteLine.Sum(l => l.AllowanceCharge?.Sum(ac => ac.ChargeIndicator.Value ? ac.Amount.Value : 0M));
            ...
            var vSubtotalNG = Math.Abs(subtotalPU - CreditNote.LegalMonetaryTotal.TaxExclusiveAmount.Value);
            ...
            var iSubtotalBG = CreditNote.LegalMonetaryTotal.TaxExclusiveAmount.Value.ToString("#,###,##0.##");
            ...
            var totalImpuesto = CreditNote.TaxTotal?.Sum(t => t.TaxAmount.Value);
            ...
            var totalMI = CreditNote.LegalMonetaryTotal.TaxInclusiveAmount.Value;
            ...
            var descuentoGlobal = CreditNote.LegalMonetaryTotal.AllowanceTotalAmount?.Value;
            ...
            var recargoGlobal = CreditNote.LegalMonetaryTotal.ChargeTotalAmount?.Value;
            ...
            var anticipo = CreditNote.LegalMonetaryTotal.PrepaidAmount?.Value ?? 0;
            ...
            var vTotalAP = CreditNote.LegalMonetaryTotal.PayableAmount.Value;
            ...
            var moneda = CreditNote.DocumentCurrencyCode.Value;
        ...
        private void agregarLineas(Document document, int desdeIndex, int cantidad)
        {
            ...
            for (int i = 0; i < cantidad; i++)
            {
                ...
                if (x < totalLineas)
                {
                    var linea = CreditNote.CreditNoteLine[x];
                    ...
                    var vxCantidad = linea.CreditedQuantity.Value.ToString("0.##");
